...
plot3 <- function() {
    ...
    plot3data <- dataset[!is.na(dataset$Global_active_power), ]
    

    plot3data$DT <- as.POSIXct(paste(plot3data$Date, plot3data$Time), 
                               format='%d/%m/%Y %H:%M:%S')
    ...
    with(subset(plot3data), points(DT, Sub_metering_1, col = "black", type="l")) 
    with(subset(plot3data), points(DT, Sub_metering_2, col = "red", type="l")) 
    with(subset(plot3data), points(DT, Sub_metering_3, col = "blue", type="l")) 
    
    legend("topright",
           text.width=strwidth("  Sub_metering_1  "),
           xjust=0.5,
           cex=1.0,  # rel text size
           x.intersp=1.1,
           y.intersp=1.3,
           lty = c(1, 1, 1),
           pch = c(NA, NA, NA), 
    ...
    dev.copy(png,"plot3.png", width = 480, height = 480, type="quartz", bg="white")
    dev.off()
}
