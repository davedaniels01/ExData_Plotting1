...
plot2 <- function() {
    ...
    plot2data <- dataset[!is.na(dataset$Global_active_power), ]
    ...
    plot2data$DT <- as.POSIXct(paste(plot2data$Date, plot2data$Time), 
                               format='%d/%m/%Y %H:%M:%S')
    
    # plot data (date and power) 
    with (plot2data, 
    ...
    dev.copy(png,"plot2.png", width = 480, height = 480, type="quartz", bg="white")
    dev.off()
